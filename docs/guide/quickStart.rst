...
.. dropdown:: Read and display the map
   :open:
   ...
   .. tip::
      ...
      ``read_sky_map`` returns a **flattened** map (a single NSIDE). Recent IGWN
      alerts are often multi-resolution (MOC): in that case use
      ``read_sky_map(url, moc=True)``. |tilepy| handles both formats
      automatically.
...
Everything depends on the **NSIDE** parameter (a power of 2):

* number of pixels: :math:`N_\mathrm{pix} = 12\,\mathrm{NSIDE}^2`;
* typical pixel size: :math:`\theta_\mathrm{pix} \approx 58.6° / \mathrm{NSIDE}`.

.. list-table:: Orders of magnitude
   :header-rows: 1
   :widths: 20 30 30
...
.. important::
   ...
   |tilepy| works with **two** resolutions, configurable in the ``.ini`` file:

   * ``HRnside`` (high resolution) to integrate the probability inside a field of
     view precisely;
   * ``reducedNside`` (low resolution) to scan the sky quickly.
...
.. important::
   ...
   In **2D** mode tilepy does not use the galaxy catalog, so you avoid downloading
   GLADE+ (``Gladeplus.h5``), which is several GB. It is only needed in 3D mode.
   Pass ``None`` for both the dataset directory and the catalog name.
