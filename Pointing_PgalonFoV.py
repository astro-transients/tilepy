...
parser.add_argument('-params', metavar='parameters.ini', default="./configs/PGalinFoV.ini",
...
print('Time of the event is:',InputTimeList['Time'][j])
...
#InputObservationFile='obstime.txt'
...
dirName='./output/%s/PGalonFoV' % name
...
try:
    ...
    outfilename = dirName+'/SimuGAL' + str("{:03d}".format(j)) + '.txt'
...
SuggestedPointings,cat,FOV,nside=PGalonFoV(GWFile,galFile,InputObservationList,InputTimeList['Observatory'][j],InputList['Distance'][j],InputList['DistMax'][j],InputList['DistMin'][j], ObservationTime0,parameters,dirName)
end = time.time()
print('Execution time: ', end - start)

pointingsFile = '%s/SuggestedPointings_GALOptimisation.txt' % dirName
...
pointingsFile = '%s/SuggestedPointings_GALOptimisation.txt' % dirName
RAarray, DECarray, P_GWarray, P_GALarray= np.genfromtxt(pointingsFile, usecols=(2, 3,4,5), skip_header=1, unpack=True)  # ra, dec in degrees
preDefWind= np.genfromtxt(pointingsFile, usecols=(6), skip_header=1, unpack=True,dtype=int)  # ra, dec in degrees

SuggestedPointings = Table([RAarray, DECarray, P_GWarray, P_GALarray,preDefWind], names=('RA(deg)','DEC(deg)','PGW','Pgal','preDefWind'))
'''
...
if(len(SuggestedPointings)!=0):
    ...
    if Found==False:
        nP = -1
        ProduceSummaryFile(InputList, InputObservationList, predefWind, nP, j, 'GW', totalPGW)
    ...
    fitsfile = fits.open(filepath)
    luminosity = fitsfile[0].header['EISO']
    outfilename = 'ICRCsimulation/SimuGAL' + str("{:03d}".format(j)) + '.txt'
    f = open(outfilename, 'w')
