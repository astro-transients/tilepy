...
def ObtainCumulativeProbabilityPlot(folder_path, event_name, WhatToPlot, interactive=False):
    ...
    for index, (observatory, observatory_data) in enumerate(grouped_data):
        ...
        for xv, yvs, xs, yv in zip(x_values, y_values_shifted, x_shifted, y_values):
            if not legend_added[observatory]:
                plt.plot([xv, xs], [yvs, yv], marker='.', linestyle='-', color=colors[index], label=observatory)
...
def ObtainCumulativeProbabilityPlotLog(folder_path, event_name, WhatToPlot, interactive=False):
    ...
    for index, (observatory, observatory_data) in enumerate(grouped_data):
        ...
        for i, (xv, yvs, xs, yv) in enumerate(zip(x_values, y_values_shifted, x_shifted, y_values)):
            if i == 0:
