...
class SkyMap:
    ...
    def getPixIdArea(self, fraction_localisation):
        ...
        prob = self.raw_map_prob_density[sorted_pixel_id]*self.raw_map_prob_density.pixarea(sorted_pixel_id)
        summed_probability = np.cumsum(prob)

        self.pix_id_area_cache[fraction_localisation] = sorted_pixel_id[summed_probability <= fraction_localisation]
    ...
    def getMap(self, mapType, nside, scheme='ring'):
        ...
        if mapType == 'prob_density':
            ...
            self.rasterized_map_cache[cache_entry] = SkyCoord(ra=lon*u.deg, dec=lat*u.deg)
        else:
