...
from gammapy.image.models import SkyGaussian, SkyPointSource,SkyDiffuseConstant,SkyDiffuseMap,SkyDisk
...
def CubeSimulationMultiObservations(InputListrun,InputListMergerID,pointing,nP,Source,IRFfilename,InputObservationTime,dirName):

    absorption = Absorption.read_builtin('dominguez')
    filepath='/Users/mseglar/Documents/CurrentPhD/CTA/GWCosmos/GammaCatalogV1.0/'+InputListrun + '_' + InputListMergerID.split('r')[-1] + ".fits"
...
def CubeSimulationMultiObservations_dynamicSpectrum(InputListrun,InputListMergerID,pointing,nP,Source,IRFfilename,InputObservationTime,dirName):
...
def CubeSimulationSingleObservation_dynamicSpectrum(InputListrun,InputListMergerID,pointing,nP,Source,IRFfilename,InputObservationTime,dirName):

    absorption = Absorption.read_builtin('dominguez')
    filepath='../dataset/GammaCatalogV1.0/'+InputListrun + '_' + InputListMergerID.split('r')[-1] + ".fits"
...
def CubeSimulationSingleObservation_singleSpectrum(InputListrun,InputListMergerID,pointing,nP,Source,IRFfilename,InputObservationTime,dirName):
    ...
    sky_model = SkyModel(spatial_model=spatial_model, spectral_model=grb.spectral_model[0])
    npred, counts, background, exposure= PointingSimulation(sky_model,axis,geom3D,pointing,np.int(InputObservationTime['Interval'])*u.second,offset,irfs)
    Cube3DMap.data=counts
    Background3DMap.data=background.data
    Exposure3DMap.data=exposure.data
